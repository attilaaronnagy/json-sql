...
describe('Update', function() {
  describe('modifier', function() {
    ...
    it('should be ok with default(`$set`)', function() {
      ...
      expect(result.query).to.be.equal('update "users" set "name" = $p1, "age" = 16, ' + '"lastvisit" = null, "active" = false, "date" = $p2;');
  ...
  describe('with', function() {
    it('should be ok', function() {
      ...
      expect(result.query).to.be.equal('with "t_1" as (select * from "t_1") update "users" ' + 'set "age" = "age" - 3;');
      expect(result.values).to.be.eql({});
    });
  });
  ...
  describe('returning', function() {
    it('should be ok', function() {
      ...
      expect(result.query).to.be.equal('update "users" set "age" = "age" - 3 returning "users".*;');
      expect(result.values).to.be.eql({});
    });
  });
});
