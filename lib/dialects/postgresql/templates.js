...
const templateChecks = require('../../utils/templateChecks');

module.exports = dialect => {
  const explainFormatRegExp = /^(text|xml|json|yaml)$/i;
  ...
  const selectTemplate = dialect.templates.get('select');
  selectTemplate.pattern = selectTemplate.pattern.replace('{distinct}', '{distinct} {distinctOn}');

  const parentSelectValidate = selectTemplate.validate;
  selectTemplate.validate = (type, params) => {
